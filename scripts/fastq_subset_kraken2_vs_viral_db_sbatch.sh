...
#SBATCH -o /shared/ifbstor1/projects/2314_medbioinfo/radoslaw/MedBioinfo/analyses/kraken2/outputs_kraken/slurm.%A.%a.out   # standard output (STDOUT) redirected to these files (with Job ID and array ID in file names) HAS TO EXIST BEFORE BEING SUBMITTED!!!!!!!!!
#SBATCH -e /shared/ifbstor1/projects/2314_medbioinfo/radoslaw/MedBioinfo/analyses/kraken2/outputs_kraken/slurm.%A.%a.err   # standard error  (STDERR) redirected to these files (with Job ID and array ID in file names)
...
input_file="/shared/ifbstor1/projects/2314_medbioinfo/radoslaw/MedBioinfo/data/sra_fastq/ERR6913349"
#accnum_file="/shared/ifbstor1/projects/2314_medbioinfo/radoslaw/MedBioinfo/analyses/rgrochowski_run_accessions.txt"
output_file=${workdir}"analyses/kraken2/kraken2"
...
input_file1=${input_file}_1.fastq.gz
input_file2=${input_file}_2.fastq.gz

# Start work
echo "srun kraken2 --paired --threads ${SLURM_CPUS_PER_TASK} --db ${db}  --gzip-compressed --output ${output_file}.out --report ${output_file}_report.tsv  ${input_file}_1.fastq.gz ${input_file}_2.fastq.gz"
srun kraken2 --paired --threads ${SLURM_CPUS_PER_TASK} --db ${db}  --gzip-compressed --output ${output_file}.out --report ${output_file}_report.tsv  ${input_file}_1.fastq.gz ${input_file}_2.fastq.gz 
