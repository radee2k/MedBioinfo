...
#SBATCH --cpus-per-task=10           # nb of cpu (in fact cores) to reserve for each task /!\ job killed if commands below use more cores
#SBATCH --mem=50GB                  # amount of RAM to reserve for the tasks /!\ job killed if commands below use more RAM
...
# Start work
srun --job-name=${accnum} blastn -num_threads ${SLURM_CPUS_PER_TASK} -query ${input_file}_10.fasta -db ${db} -evalue 1E-6 -outfmt 6 -perc_identity 75 -out ${output_file}_10.out -max_target_seqs 5
