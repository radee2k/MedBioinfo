...
# Start work
srun  --job-name=rg_kraken2 kraken2 --paired --threads ${SLURM_CPUS_PER_TASK} --db ${db}  --gzip-compressed --output ${output_file}.out --report ${output_file}_report.tsv  ${input_file}_1.fastq.gz ${input_file}_2.fastq.gz
...
srun --job-name=rg_bracken bracken -d ${db} -i ${output_file}_report.tsv -o ${output_file}_bracken.tsv -w  ${output_file}_bracken_report.tsv
